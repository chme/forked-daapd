...
static void
online_source_history_update(
    const struct online_source *src, int id, uint32_t request_hash, int response_code, const char *artwork_url)
{
  struct online_search_history *history = src->search_history;
...
online_source_search(const struct online_source *src, struct artwork_ctx *ctx)
{
  ...
  ret = online_source_response_parse(
      &artwork_url, src, client.input_body, ctx->req_params.max_w, ctx->req_params.max_h);
