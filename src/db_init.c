...
#define Q_QUEUE_VERSION "INSERT INTO admin (key, value) VALUES ('queue_version', '0');"

#define Q_SCVER_MAJOR "INSERT INTO admin (key, value) VALUES ('schema_version_major', '%d');"
#define Q_SCVER_MINOR "INSERT INTO admin (key, value) VALUES ('schema_version_minor', '%02d');"
...
#define I_RESCAN "CREATE INDEX IF NOT EXISTS idx_rescan ON files(path, db_timestamp);"

#define I_FNAME "CREATE INDEX IF NOT EXISTS idx_fname ON files(disabled, fname COLLATE NOCASE);"

#define I_SONGARTISTID "CREATE INDEX IF NOT EXISTS idx_sari ON files(songartistid);"
...
#define I_STATEMKINDSARI "CREATE INDEX IF NOT EXISTS idx_state_mkind_sari ON files(disabled, media_kind, songartistid);"

#define I_STATEMKINDSALI "CREATE INDEX IF NOT EXISTS idx_state_mkind_sali ON files(disabled, media_kind, songalbumid);"

/* Used by Q_BROWSE_ALBUM */
#define I_ALBUM "CREATE INDEX IF NOT EXISTS idx_album ON files(disabled, album_sort, album, media_kind);"
...
#define I_COMPOSER "CREATE INDEX IF NOT EXISTS idx_composer ON files(disabled, composer_sort, composer, media_kind);"

/* Used by Q_BROWSE_GENRES */
#define I_GENRE "CREATE INDEX IF NOT EXISTS idx_genre ON files(disabled, genre, media_kind);"

/* Used by Q_PLITEMS for smart playlists */
#define I_TITLE "CREATE INDEX IF NOT EXISTS idx_title ON files(disabled, title_sort, media_kind);"

#define I_FILELIST "CREATE INDEX IF NOT EXISTS idx_filelist ON files(disabled, virtual_path, time_modified);"

#define I_FILE_DIR "CREATE INDEX IF NOT EXISTS idx_file_dir ON files(disabled, directory_id);"

#define I_DATE_RELEASED                                                                                                \
  "CREATE INDEX IF NOT EXISTS idx_date_released ON files(disabled, date_released DESC, media_kind);"

#define I_PL_PATH "CREATE INDEX IF NOT EXISTS idx_pl_path ON playlists(path);"

#define I_PL_DISABLED                                                                                                  \
  "CREATE INDEX IF NOT EXISTS idx_pl_disabled ON playlists(disabled, type, virtual_path, db_timestamp);"

#define I_PL_DIR "CREATE INDEX IF NOT EXISTS idx_pl_dir ON files(disabled, directory_id);"

#define I_FILEPATH "CREATE INDEX IF NOT EXISTS idx_filepath ON playlistitems(filepath ASC);"

#define I_PLITEMID "CREATE INDEX IF NOT EXISTS idx_playlistid ON playlistitems(playlistid, filepath);"

#define I_GRP_PERSIST "CREATE INDEX IF NOT EXISTS idx_grp_persist ON groups(persistentid);"

#define I_PAIRING "CREATE INDEX IF NOT EXISTS idx_pairingguid ON pairings(guid);"

#define I_DIR_VPATH "CREATE INDEX IF NOT EXISTS idx_dir_vpath ON directories(disabled, virtual_path);"

#define I_DIR_PARENT "CREATE INDEX IF NOT EXISTS idx_dir_parentid ON directories(parent_id);"

#define I_QUEUE_POS "CREATE INDEX IF NOT EXISTS idx_queue_pos ON queue(pos);"

#define I_QUEUE_SHUFFLEPOS "CREATE INDEX IF NOT EXISTS idx_queue_shufflepos ON queue(shuffle_pos);"
