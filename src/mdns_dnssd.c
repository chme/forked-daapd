...
mdns_init(void)
{
  ...
  mdns_ev_main = event_new(evbase_main, fd, EV_PERSIST | EV_READ, mdns_event_cb, NULL);
  if (!mdns_ev_main)
    {
...
static void
mdns_register_callback(DNSServiceRef sdRef, DNSServiceFlags flags, DNSServiceErrorType errorCode, const char *name,
...
mdns_register(char *name, char *regtype, int port, char **txt)
{
  ...
  err = DNSServiceRegister(&(s->sdref), kDNSServiceFlagsShareConnection, 0, name, regtype, NULL, NULL, htons(port),
      TXTRecordGetLength(&(s->txtRecord)), TXTRecordGetBytesPtr(&(s->txtRecord)), mdns_register_callback, NULL);
...
static void
mdns_record_callback(
    DNSServiceRef sdRef, DNSRecordRef RecordRef, DNSServiceFlags flags, DNSServiceErrorType errorCode, void *context)
{
  struct mdns_record *r;
...
static int
mdns_register_record(uint16_t rrtype, const char *name, uint16_t rdlen, const void *rdata)
{
  struct mdns_record *r;
  DNSServiceErrorType err;
  ...
  err = DNSServiceRegisterRecord(mdns_sdref_main, &r->recRef, kDNSServiceFlagsShared, 0, r->name, r->rrtype,
      kDNSServiceClass_IN, rdlen, rdata, 0, mdns_record_callback, r);
...
mdns_cname(char *name)
{
  ...
  return mdns_register_record(kDNSServiceType_CNAME, name, (uint16_t)ret, rdata);
}

static void
mdns_browse_call_cb(struct mdns_addr_lookup *lu, const char *hostname, const struct sockaddr *address)
{
  ...
  lu->rs->mb->cb(
      lu->rs->service, lu->rs->regtype, lu->rs->domain, hostname, address->sa_family, addr_str, lu->port, &lu->txt_kv);
}

static void
mdns_lookup_callback(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex, DNSServiceErrorType errorCode,
...
static int
mdns_addr_lookup_start(struct mdns_resolver *rs, uint32_t interfaceIndex, const char *hosttarget, uint16_t port,
    uint16_t txtLen, const unsigned char *txtRecord)
{
  ...
  for (i = 0; TXTRecordGetItemAtIndex(txtLen, txtRecord, i, sizeof(key), key, &valueLen, (const void **)&value)
  ...
  err = DNSServiceGetAddrInfo(&lu->sdref, kDNSServiceFlagsShareConnection, interfaceIndex, rs->mb->protocol, hosttarget,
...
mdns_resolve_timeout_cb(evutil_socket_t fd, short flags, void *uuid)
{
  ...
  for (mb = mdns_browsers; mb && !rs; mb = mb->next)
    for (rs = mb->resolvers; rs; rs = rs->next)
      if (rs->uuid == uuid)
	{
	  DPRINTF(E_DBG, L_MDNS, "Resolve finished for '%s' type '%s' interface %d\n", rs->service, rs->regtype,
	      rs->interface);
...
static void
mdns_resolve_callback(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
    ...
    const unsigned char *txtRecord, void *context)
{
  ...
  if (errorCode != kDNSServiceErr_NoError)
  ...
  else
    {
      ...
      mdns_addr_lookup_start(rs, interfaceIndex, hosttarget, port, txtLen, txtRecord);
    }
}

static int
mdns_resolve_start(struct mdns_browser *mb, uint32_t interfaceIndex, const char *serviceName, const char *regtype,
    const char *replyDomain)
{
  ...
  err = DNSServiceResolve(&(rs->sdref), kDNSServiceFlagsShareConnection, interfaceIndex, serviceName, regtype,
      replyDomain, mdns_resolve_callback, rs);
  if (err != kDNSServiceErr_NoError)
    {
      DPRINTF(E_LOG, L_MDNS, "Failed to create service resolver.\n");
      rs->sdref = NULL;
...
static void
mdns_resolve_cancel(const struct mdns_browser *mb, uint32_t interfaceIndex, const char *serviceName,
    const char *regtype, const char *replyDomain)
{
  ...
  for (rs = mb->resolvers; rs; rs = rs->next)
    {
      if ((rs->interface == interfaceIndex) && (!strcasecmp(rs->service, serviceName))
          && (!strcmp(rs->regtype, regtype)) && (!strcasecmp(rs->domain, replyDomain)))
	{
...
static void
mdns_browse_callback(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex, DNSServiceErrorType errorCode,
    const char *serviceName, const char *regtype, const char *replyDomain, void *context)
{
  ...
  if (flags & kDNSServiceFlagsAdd)
    {
      DPRINTF(E_DBG, L_MDNS, "Bonjour Browser: NEW service '%s' type '%s' interface %d\n", serviceName, regtype,
          interfaceIndex);
      mdns_resolve_start(mb, interfaceIndex, serviceName, regtype, replyDomain);
    }
  else
    {
      DPRINTF(E_DBG, L_MDNS, "Bonjour Browser: REMOVE service '%s' type '%s' interface %d\n", serviceName, regtype,
          interfaceIndex);
      mdns_resolve_cancel(mb, interfaceIndex, serviceName, regtype, replyDomain);
...
mdns_browse(char *regtype, mdns_browse_cb cb, enum mdns_options flags)
{
  ...
  err = DNSServiceBrowse(&(mb->sdref), kDNSServiceFlagsShareConnection, 0, regtype, NULL, mdns_browse_callback, mb);
  if (err != kDNSServiceErr_NoError)
    {
